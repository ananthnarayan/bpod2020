...
run_remote_workload_bdb() {
    ...
    namenode=`ps -elf | grep namenode | grep -v "grep" | tail -n 1 | tr -s " " | cut -d ' ' -f 4`
    datanode=`ps -elf | grep datanode | grep -v "grep" | tail -n 1 | tr -s " " | cut -d ' ' -f 4`
    nodemanager=`ps -elf | grep nodemanager | grep -v "grep" | tail -n 1 | tr -s " " | cut -d ' ' -f 4`
    resourcemanager=`ps -elf | grep resourcemanager | grep -v "grep" | tail -n 1 | tr -s " " | cut -d ' ' -f 4`
    secondarynamenode=`ps -elf | grep secondarynamenode | grep -v "grep" | tail -n 1 | tr -s " " | cut -d ' ' -f 4`

     
    echo -e "Perfpid: $perfpid Bash: $bashpid Java: $javapid Hadoop: $namenode,$datanode,$nodemanager,$resourcemanager,$secondarynamenode"
    pidstat -h -r -s -T ALL -p $namenode,$datanode,$nodemanager,$resourcemanager,$secondarynamenode $sar_delay > $pidstat_file &  
    time=`cat timeout` 
    wait $timepid
    echo "killing sar"
